...
Будучи PHP разработчиком вы, вероятно, пишите соответствующую библиотеку каждый раз, когда необходимо
...
Она предоставит вам удобные инструменты, которые помогут объединить конфигурацию (или "метаданные") из 
...
В этой главе я решил не касаться более этого вопроса, так как использование аннотаций для сбора метаданных 
...
разве что самим не делать опечаток.
...
соответствует ли его путь указанному шаблону и если имеет ли предыдущий URL тот же домен, что и текущий.
...
В следующем примере я хочу либо предотвратить/разрешить выполнение контроллера, основываясь на наличии неких 
...
Этот вариант содержит некоторое количество довольно сложно читаемого кода, и, разумеется, не очень хорошо копировать
...
действие контроллера помечено как "downloadable". Давайте сделаем это возможным припомощи аннотации @DownloadAs:
...
наличие аннотации @DownloadAs. Если она найдена, мы сохраним имя файла в виде атрибует текущего объекта
Request:
...
Не забудьте зарегисстрировать этот класс в качестве сервиса и объявить его слущателем событий kernel.controller 
и kernel.response
